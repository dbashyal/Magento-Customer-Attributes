...
class Technooze_Tcustomerattributes_Adminhtml_IndexController extends Mage_Adminhtml_Controller_Action {
    ...
    public function indexAction() {
        $this->_initAction()->_addContent($this->getLayout()->createBlock('tcustomerattributes/adminhtml_Grid'));
        $this->renderLayout();
    }
    ...
    public function editAction() {
        ...
        if ($id) {
            ...
            if (!$model->getId()) {
                Mage::getSingleton('adminhtml/session')->addError(
                        Mage::helper('tcustomerattributes')->__('This attribute no longer exists'));
                $this->_redirect('*/*/');
                return;
            }
            ...
            if ($model->getEntityTypeId() != $this->_entityTypeId) {
                Mage::getSingleton('adminhtml/session')->addError(
                        Mage::helper('tcustomerattributes')->__('This attribute cannot be edited.'));
                $this->_redirect('*/*/');
                return;
            }
        ...
        $item = $id ? Mage::helper('tcustomerattributes')->__('Edit Customer Attribute') : Mage::helper('tcustomerattributes')->__('New Customer Attribute');
        ...
        $this
                ->_addContent($this->getLayout()->createBlock('tcustomerattributes/adminhtml_edit'))
                ->_addLeft($this->getLayout()->createBlock('tcustomerattributes/adminhtml_edit_tabs'))
        ;
    ...
    public function validateAction() { 
        ...
        if ($attribute->getId() && !$attributeId) {
            Mage::getSingleton('adminhtml/session')->addError(
                Mage::helper('tcustomerattributes')->__('Attribute with the same code already exists'));
    ...
    public function saveAction() {
        ...
        if ($data) {
            ...
            $model = Mage::getModel('tcustomerattributes/tcustomerattributes');
            /* @var $helper Mage_Catalog_Helper_Product */
            $helper = Mage::helper('tcustomerattributes');
            ...
            if ($id) {
                ...
                if ($model->getEntityTypeId() != $this->_entityTypeId) {
                    $session->addError(
                        Mage::helper('tcustomerattributes')->__('This attribute cannot be updated.'));
            ...
            try {
                ...
                $session->addSuccess(
                    Mage::helper('tcustomerattributes')->__('The product attribute has been saved.'));
    ...
    protected function _filterPostData($data)
    {
        if ($data) {
            ...
            $helperCatalog = Mage::helper('tcustomerattributes');
            //labels
    ...
    public function deleteAction()
    {
        if ($id = $this->getRequest()->getParam('attribute_id')) {
            ...
            if ($model->getEntityTypeId() != $this->_entityTypeId) {
                Mage::getSingleton('adminhtml/session')->addError(
                    Mage::helper('tcustomerattributes')->__('This attribute cannot be deleted.'));
                $this->_redirect('*/*/');
                return;
            }
            ...
            try {
                ...
                Mage::getSingleton('adminhtml/session')->addSuccess(
                    Mage::helper('tcustomerattributes')->__('The product attribute has been deleted.'));
                $this->_redirect('*/*/');
                return;
            }
            catch (Exception $e) {
        ...
        Mage::getSingleton('adminhtml/session')->addError(
            Mage::helper('tcustomerattributes')->__('Unable to find an attribute to delete.'));
        $this->_redirect('*/*/');
    }
}